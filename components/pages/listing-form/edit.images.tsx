	...
	return (
		<section className="bg-card rounded-xl p-6 shadow-sm">
			<div className="flex items-center justify-between">
				<h3 className="text-base font-semibold">{t('images')}</h3>
					...
					return (
						...
						>
							...
							<Button
								...
								size="icon-sm"
								disabled={isPending}
								title={isRemoved ? tCommon('restore') : tCommon('delete')}
								aria-label={isRemoved ? t('imageRestore') : t('imageRemove')}
								onClick={() =>
			...
			>
				...
				<div className="flex flex-col items-center justify-center space-y-2">
					...
					>
						{isFull
							? t('imagesFull')
							: t('imagesAdd', { count: availableCount })}
					</p>
					<p className="text-muted-foreground text-xs">{t('imagesFormats')}</p>
				</div>
			</div>
			...
			{hasChanges && (
				<div className="mt-4 flex items-center justify-between gap-3 border-t pt-4">
					<p className="text-muted-foreground text-xs">
						{removedIds.length > 0 &&
							t('imagesWillRemove', { count: removedIds.length })}
						{newPreviews.length > 0 &&
							t('imagesWillAdd', { count: newPreviews.length })}
					</p>
			...
			{totalCount === 0 && (
				<p className="text-destructive mt-4 text-sm">{t('imagesMinOne')}</p>
			)}
